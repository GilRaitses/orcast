...
> **SUPERSEDED for the forecast-skill path by SYNTHESIS_signal_modeling.md (2026-06-27) + G3.** The
> "What the gate state says we need" ordering below is stale: the binding +0.144 lever is **summer
> (Jun-Sep) PRESENCE-DAYS from new in-region nodes** (SYN section 1 / graduation TB1, TB4), NOT "a real
> Chinook prey index." The Albion Chinook feed is already real/complete/stock-aligned (G3), so blocker
> #2 below is met -- prey-series refresh (TB3) is SUPPORTING only and cannot move L3 on its own; L3 stays
> WITHHELD until the presence-day / LOYO power bar is met. **CUTI/BEUTI upwelling is NO-GO / WITHHELD**
> (the product covers 31-47 N; the region is ~48.5 N, out of coverage -- S2 section 3 #7). Read
> SYNTHESIS_signal_modeling.md before using this register for the skill path.
...
> NOTE (2026-06-27, supersedes the order below per SYN/G3): the binding lever is **summer presence-days
> from new in-region nodes** (TB1 Port Townsend/Bush Point, TB4 Boundary Pass), not a prey index.
> Re-read: (1) new-node summer presence-days; (2) multi-station acoustic coverage of the existing feed;
> (3) held-out visual validation + effort-corrected `s_space`. The "real Chinook prey index" is already
> met (Albion, G3) and is supporting-only.

1. Multi-station acoustic coverage (the L2 substrate the fit is starving for).
2. ~~A real Chinook prey index~~ (MET: Albion is real/stock-aligned per G3; supporting-only, does not
   unblock M-L3 on its own -- L3 is presence-day/power-gated, not feed-gated).
...
| NOAA satellite chl-a (CoastWatch/ERDDAP) ~~+ CUTI/BEUTI upwelling~~ | WL-PREYBASE | indicator (productivity) | Bottom-up driver of Chinook growth/survival; multi-year lag to adult Chinook. **CUTI/BEUTI WITHHELD / NO-GO** -- the product covers 31-47 N only; the region (~48.5 N) is out of coverage (S2 section 3 #7). If an upwelling term is ever wanted, require a locally computed in-region index, NOT NOAA CUTI/BEUTI. |
