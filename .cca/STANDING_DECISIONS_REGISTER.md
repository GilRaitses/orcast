...
- **Rationale:** Matches the original design (`DEPLOY_VERCEL.md:27`); Next.js auto-detect inside `web/` is sufficient. **Root-cause correction (2026-06-26 deploy verification):** the SD-H handoff premise (root `vercel.json` "conflicting with Root Directory=`web`") was incomplete. The dashboard Root Directory was actually `.` on both projects (never `web`), so the root `vercel.json`'s `cd web` shim was load-bearing for the CLI flow, not the bug. The real reasons Git builds were red: (a) the `web/` app (package.json, lockfile, config, most source) was never committed — only `vercel` CLI uploads of local files deployed it; (b) a duplicate misconfigured project `orcast` (FastAPI preset, Root `.`, Git-connected) was erroring on every push. Fixed by: committing the app (`8e5c6ee`), setting Root Directory=`web`, deleting the duplicate `orcast` project, and Git-connecting `orcast-h0`.
- **Status:** ratified + **verified GREEN in production** (2026-06-26). Git deployment `gfgdba0t5` (commit `8e5c6ee`) holds the `orcast-h0.vercel.app` production alias; `GET /` 200, `POST /api/be/api/interest` 200, `GET /api/be/api/gates` 200.
- **Resolved:** 2026-06-26 SD-H remediation + deploy verification. Supersedes SD-011-OLD.
...
- **O-1 (deploy): RESOLVED 2026-06-26.** Root Directory=`web` set on `orcast-h0`; duplicate `orcast` (FastAPI) project deleted; `orcast-h0` Git-connected to `GilRaitses/orcast`@`main`; the `web/` app committed (`8e5c6ee`). Git deployment `gfgdba0t5` is GREEN and holds the `orcast-h0.vercel.app` production alias (verified `GET /` 200, `POST /api/be/api/interest` 200, `GET /api/be/api/gates` 200). The `web/.vercel/project.json` local CLI link remains; future deploys should go through Git push.
...
- **O-4 (claim drift remediation):** The P1 drift listed under SD-001/002/005/006/011/021 (stale 91% baseline, page counts, team voice in `web/lib/glossary.ts`, deploy-canon docs) are live violations of ratified rows. SD is documentation-only; a follow-up remediation wave should fix them. `DEVPOST_DRAFT.md` and the demo are do-not-touch and excluded. **New (2026-06-26 deploy verify):** the App Runner backend `/api/interest` response returns "Thanks — you'll hear from us when the papers are ready." — team voice that violates SD-006 / SD-028 (interest copy should be single-author "hear from me"). Lives in the backend (`src/aws_backend/**`), not `web/`; fix in the remediation wave.
