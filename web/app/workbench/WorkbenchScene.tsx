...
// B-side acoustic + behavior WORKBENCH (promoted /workbench route).
//
// This is the dedicated B-side promotion of the verified composition built and
// host-verified at the /slice sandbox reference. It wires the same four disjoint
// net-new lanes into the one real vertical slice, end to end:
//
...
//
// It imports the four lanes' PUBLIC barrels only; it edits no lane file and no
// convergence file, and all chrome styling lives in workbench.module.css
// (namespaced bsw-* with LGC-token fallbacks). Deterministic headless framing
// via a SINGLE query param (the render host passes the route as one arg):
//   /workbench                  establishing column shot, presence-positive, paused
//   /workbench?view=hydrophone   hydrophone POV (drives the Camera Director)
//   /workbench?view=topdown      top-down station overview (Camera Director)
//   /workbench?t=30              seek the shared playhead to 30 s (paused)
...
function SceneContent({
  ...
  useEffect(() => {
    ...
    if (view === "establish") {
      // Establishing column shot: 3/4 side, framing the seabed rig up through the
      // water column to the surface float + the orca above it.
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setPoolReady(true);
    })().catch((e) => console.error("workbench reenactment failed", e));
...
export default function WorkbenchScene() {
  ...
  useEffect(() => {
    ...
    createSpectroTimeline({
      ...
      .catch((e) => {
        console.error("workbench spectro timeline failed", e);
  ...
  return (
    <div className={styles["bsw-root"]}>
      <div className={styles["bsw-canvasWrap"]}>
        <Canvas
      ...
      <div className={styles["bsw-hud"]} ref={hostRef} />

      {/* View selector (drives BST's Camera Director for the POV views). */}
      <div className={`${styles["bsw-views"]} ${styles["bsw-glass"]}`}>
        {(["establish", "hydrophone", "topdown"] as View[]).map((v) => (
          <button
            key={v}
            type="button"
            onClick={() => onView(v)}
            className={`${styles["bsw-segment"]} ${
              view === v ? styles["bsw-segmentActive"] : ""
            }`}
          >
            {v}
          </button>
      ...
      <div className={`${styles["bsw-estimate"]} ${styles["bsw-glass"]}`}>
        <div className={styles["bsw-estimateLabel"]}>
          {driverState.label || "loading acoustic estimate…"}
        </div>
        <div className={styles["bsw-muted"]}>
          orcas shown: {driverState.presence ? driverState.count : 0} · presence-gated
        </div>
        <div className={`${styles["bsw-muted"]} ${styles["bsw-spaced"]}`}>{ATTRIBUTION}</div>
        <a href={STREAM_URL} target="_blank" rel="noreferrer" className={styles["bsw-link"]}>
          Live-listen: orcasound-lab
        </a>
      </div>

      {/* Honesty legend + mandatory representativeness label. */}
      <div className={`${styles["bsw-legend"]} ${styles["bsw-glass"]}`}>
        <strong>Orcasound Lab workbench</strong> ({LAT}, {LNG}) · t = {fmt(driverState.t)} ·{" "}
        {status}
        <div className={styles["bsw-spaced"]}>
          measured: audio · spectrogram · SRKW DTAG motion
        </div>
        <div>modeled: equipment mesh · acoustic inference · 3D placement</div>
        <div className={styles["bsw-repr"]}>
