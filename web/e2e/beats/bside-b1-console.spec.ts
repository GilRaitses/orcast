...
// pick the gates starter prompt, send a turn, and hold on the live orchestrator trace.
...
const GATES_PROMPT = "Which gates block promotion right now?";
...
test("bside-b1 ask the console", async ({ page }) => {
  ...
  // Show the chat UI in use: click the gates starter chip, then send the turn.
  await page.getByRole("button", { name: GATES_PROMPT }).click();
