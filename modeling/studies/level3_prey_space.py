...
def run() -> GateResult:
    ...
    metrics = {
        ...
        "k_salmon": (
            "withheld: needs an assembled Chinook run-timing series (WDFW/PSC/Albion/Fraser/"
            "Bonneville) aligned with a lag scan to acoustic presence. salmon.py currently "
            "provides only a climatology fallback."
        ),
    ...
    return GateResult(
        ...
        metrics=metrics,
        reason=(
            "s_space precursor assembled from the CAND set (density grid + depth); k_salmon and "
            "the spatial fit are withheld pending an effort model, a prey run-timing series, and "
            "held-out visual validation. No confidence promoted."
        ),
    )
