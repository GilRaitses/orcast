...
| **L3** Prey + space → field | Full spatiotemporal intensity | salmon run-timing series; bathymetry; sightings for space | Held-out skill beats recent-detection-density baseline; calibration within tolerance (reliability near diagonal; PIT ~uniform). *Gate split (2026-06-27): at the current 4-station cluster, static bathymetry enters `s_space` ONLY and is judged on spatial/visual-validation quality; it does NOT move the served acoustic TEMPORAL CV gate (SYN section 2; M3 section 0.2). The L3 binding lever is summer presence-days from new in-region nodes.* |
